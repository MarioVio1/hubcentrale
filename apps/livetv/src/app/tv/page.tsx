...
import { Play, ArrowLeft, Tv, Loader2, Volume2, VolumeX, Radio } from 'lucide-react';
...
export default function TVPage() {
  ...
  const loadList = async (url: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/m3u-fetch?url=${encodeURIComponent(url)}`);
      const data = await res.json();
      if (data.channels) {
        const parsed = data.channels.map((ch: any, i: number) => ({ ...ch, id: `ch-${i}` }));
        setChannels(parsed);
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    if (view === 'lists') {
      if (e.key === 'ArrowUp') setSelectedListIndex(i => Math.max(0, i - 1));
      else if (e.key === 'ArrowDown') setSelectedListIndex(i => Math.min(M3U_LISTS.length - 1, i + 1));
      else if (e.key === 'Enter' || e.key === ' ') loadList(currentList.url);
    } else if (view === 'channels') {
      const cols = 6;
      if (e.key === 'ArrowUp') { if (selectedChannelIndex >= cols) setSelectedChannelIndex(i => i - cols); }
      else if (e.key === 'ArrowDown') setSelectedChannelIndex(i => Math.min(filteredChannels.length - 1, i + cols));
        ...
        if (selectedChannelIndex === 0 && selectedCategoryIndex > 0) {
          ...
          setSelectedChannelIndex(0);
        } else setSelectedChannelIndex(i => Math.max(0, i - 1));
      } else if (e.key === 'ArrowRight') setSelectedChannelIndex(i => Math.min(filteredChannels.length - 1, i + 1));
      else if (e.key === 'Enter' || e.key === ' ') { if (currentChannel) playChannel(currentChannel); }
      else if (e.key === 'Escape' || e.key === 'Backspace') { setView('lists'); setChannels([]); }
    } else if (view === 'player') {
  ...
  const playChannel = (channel: TVChannel) => {
    ...
    if (isHLS) {
      ...
      video.onloadeddata = () => { setPlayerLoading(false); video.play().catch(() => {}); };
    }
  };

  const closePlayer = () => { if (hlsRef.current) hlsRef.current.destroy(); setView('channels'); };
    ...
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center">
        ...
        <div className="absolute top-4 left-4 text-white bg-black/50 px-4 py-2 rounded-lg">
          ...
          <p className="text-sm text-red-400 flex items-center gap-1"><span className="w-2 h-2 bg-red-500 rounded-full animate-pulse" /> LIVE</p>
        </div>
        {playerLoading && (<div className="absolute inset-0 flex items-center justify-center bg-black/80"><Loader2 className="w-16 h-16 animate-spin text-yellow-500" /></div>)}
        <div className="absolute bottom-4 left-4 text-white/50 text-sm">ESC = Esci | M = Muto | F = Fullscreen</div>
      </div>
    );
  }
        ...
        <div className="flex-1 p-8">
          ...
          <div className="grid gap-3 max-w-2xl">
            {M3U_LISTS.map((list, index) => (
              <div key={list.id}
                className={`p-4 rounded-xl border-2 transition-all cursor-pointer ${selectedListIndex === index ? 'border-yellow-500 bg-yellow-500/20 scale-105' : 'border-white/10 bg-white/5 hover:border-white/30'}`}
                onClick={() => { setSelectedListIndex(index); loadList(list.url); }}
              >
                <div className="flex items-center justify-between">
                  <span className="text-xl font-medium">{list.name}</span>
                  {selectedListIndex === index && <Play className="w-6 h-6 text-yellow-500 fill-yellow-500" />}
                </div>
              </div>
            ))}
          </div>
          <div className="mt-8 text-gray-500 text-sm"><p>↑↓ = Sposta • Enter = Seleziona</p></div>
        </div>
      )}

      {loading && (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center"><Loader2 className="w-16 h-16 animate-spin text-yellow-500 mx-auto mb-4" /><p className="text-xl">Caricamento canali...</p></div>
        </div>
      )}
        ...
        <div className="flex-1 flex">
          <div className="w-48 bg-[#111] border-r border-white/10 p-4 overflow-y-auto">
            ...
            <div className="space-y-1">
              {categories.map((cat, index) => (
                <div key={cat}
                  className={`p-2 rounded-lg cursor-pointer transition ${selectedCategoryIndex === index ? 'bg-yellow-500 text-black font-bold' : 'bg-white/5 hover:bg-white/10'}`}
                  onClick={() => { setSelectedCategoryIndex(index); setSelectedCategory(cat); setSelectedChannelIndex(0); }}
                >
                  {cat === 'all' ? '📺 Tutti' : cat}
                </div>
              ))}
          ...
          <div className="flex-1 p-4 overflow-hidden">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-bold">{selectedCategory === 'all' ? 'Tutti i Canali' : selectedCategory}</h2>
            ...
            <div className="grid grid-cols-6 gap-3 overflow-y-auto max-h-[calc(100vh-220px)]">
              {filteredChannels.map((channel, index) => (
                <div key={channel.id}
                  className={`aspect-video rounded-lg overflow-hidden relative transition-all cursor-pointer ${selectedChannelIndex === index ? 'ring-4 ring-yellow-500 scale-110 z-10' : 'hover:scale-105'}`}
                  style={{ backgroundColor: selectedChannelIndex === index ? '#1a1a1a' : '#111' }}
                  onClick={() => playChannel(channel)}
                >
                  <div className="absolute inset-0 flex items-center justify-center p-2">
                    {channel.logo ? (
                      <img src={channel.logo} alt={channel.name} className="max-w-full max-h-full object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                  ...
                  {selectedChannelIndex === index && (
                    <div className="absolute bottom-0 left-0 right-0 bg-yellow-500 text-black text-xs p-1 text-center font-bold truncate">{channel.name}</div>
                  )}
                </div>
              ))}
            </div>
            ...
            {currentChannel && (
              <div className="fixed bottom-0 left-0 right-0 bg-[#111] border-t border-white/10 p-4">
                <div className="flex items-center gap-4">
                  <div className="flex-1"><p className="font-bold text-lg">{currentChannel.name}</p><p className="text-gray-400">{currentChannel.group}</p></div>
