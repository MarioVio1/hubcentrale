// MangaWorldAdult scraper - similar to MangaWorld but for adult content
import * as cheerio from 'cheerio';
import { BaseScraper, type MangaInfo, type ChapterInfo, type MangaPage, type SourceConfig } from './base-scraper';

export class MangaWorldAdultScraper extends BaseScraper {
  readonly config: SourceConfig = {
    id: 'mangaworld-adult',
    name: 'MangaWorld Adult',
    lang: 'it',
    baseUrl: 'https://www.mangaworldadult.net',
    nsfw: 2, // Explicit content
  };

  /**
   * Search manga on MangaWorldAdult
   */
  async searchManga(query: string): Promise<MangaInfo[]> {
    try {
      ...
      if (!html) {
        console.log('No HTML returned from MangaWorldAdult search');
        return [];
      }
          ...
          elements.each((_, element) => {
            try {
              ...
              if (href && title && title !== '') {
                ...
                mangas.push({
                  id: mangaId,
                  extensionId: 'mangaworld-adult-ext',
      ...
      console.log(`Parsed ${mangas.length} manga from MangaWorldAdult search`);
      return mangas;
    } catch (error) {
      console.error('Error searching MangaWorldAdult:', error);
      return [];
    }
  }

  /**
   * Get popular manga from MangaWorldAdult
   */
  async getPopularManga(): Promise<MangaInfo[]> {
    try {
      ...
      if (!html) {
        console.log('Failed to fetch popular manga from MangaWorldAdult');
        return [];
      }
      ...
      console.log(`Parsed ${mangas.length} manga from MangaWorldAdult`);
      return mangas.slice(0, 50);
    } catch (error) {
      console.error('Error getting popular MangaWorldAdult:', error);
      return [];
    }
  }

  /**
   * Get latest updates from MangaWorldAdult
   */
  async getLatestManga(): Promise<MangaInfo[]> {
    try {
      ...
      if (!html) {
        console.log('Failed to fetch latest manga from MangaWorldAdult');
        return [];
      }
      ...
      console.log(`Parsed ${mangas.length} latest manga from MangaWorldAdult`);
      return mangas.slice(0, 30);
    } catch (error) {
      console.error('Error getting latest MangaWorldAdult:', error);
      return [];
    }
  }

  /**
   * Normalize URL to always have trailing slash for MangaWorldAdult
   */
  ...
  async getMangaDetails(mangaUrl: string): Promise<MangaInfo | null> {
    try {
      ...
      if (!html) {
        console.log('Failed to fetch manga details from MangaWorldAdult');
        return null;
      }
      ...
      console.log(`Fetched details for manga from MangaWorldAdult: ${title}`);
      return mangaInfo;
    } catch (error) {
      console.error('Error getting manga details from MangaWorldAdult:', error);
      return null;
    }
  }
  ...
  async getChapters(mangaUrl: string): Promise<ChapterInfo[]> {
    try {
      ...
      if (!html) {
        console.log('Failed to fetch chapters from MangaWorldAdult');
        return [];
      }
      ...
      console.log(`Parsed ${chapters.length} chapters from MangaWorldAdult`);
      return chapters;
    } catch (error) {
      console.error('Error getting chapters from MangaWorldAdult:', error);
      return [];
    }
  }

  /**
   * Get chapter pages - MangaWorldAdult specific implementation
  ...
  async getChapterPages(chapterUrl: string): Promise<MangaPage[]> {
    try {
      ...
      if (!html) {
        console.log('Failed to fetch chapter page from MangaWorldAdult');
        return [];
      }
      ...
      if (baseUrl && totalPages > 0) {
        ...
        console.log(`Successfully fetched ${pages.filter(p => p.url).length}/${totalPages} pages from MangaWorldAdult`);
        return pages;
      }
      ...
      console.error('Error getting chapter pages from MangaWorldAdult:', error);
      return [];
    }
  }
